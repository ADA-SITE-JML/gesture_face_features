...
def get_mte_scores(
    ...
    """
    Calculates and prints model transferability scores (LEEP, LogME, Regularized H-Score, Transrate)
    for all models in MODEL_ZOO. Uses: https://github.com/thuml/Transfer-Learning-Library/

    Args:
        feats (Dict[str, torch.Tensor]): Dictionary of model feature tensors.
...
def get_nleep_score(cluster: Any, random_state: int = None, verbose: bool = True) -> None:
  """
  Calculates and assigns the NLEEP score to a cluster using Gaussian Mixture Model.

  Args:
      cluster (Any): Cluster object with embedding and reducer attributes.
